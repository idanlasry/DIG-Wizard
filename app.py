...
def build_da_findings_md() -> str:
    """Builds markdown string from DA Agent findings."""
    if not st.session_state.da_findings:
        return "*DA Agent has not run yet.*"
    da = st.session_state.da_findings
    path = st.session_state.current_path or {}

    insights_md = "\n".join(f"- {i}" for i in da["key_insights"])
    stats_md = "\n".join(
        f"| {s['label']} | {s['value']} | {s['context']} |"
        for s in da["supporting_stats"]
    )
    caveat_md = f"\n> ⚠️ {da['caveats']}" if da.get("caveats") else ""

    return f"""# DA Agent Findings
...
**{da["recommended_viz"]}** — {da["viz_rationale"]}
{caveat_md}
---
...
with col_report:
    ...
    with tab_cols[3]:
        if st.button("📊 DA FINDINGS", disabled=st.session_state.da_findings is None):
            st.session_state.report_view = "da_findings"
            st.rerun()
